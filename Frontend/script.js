...
// Session auth routing check
const currentPath = window.location.pathname;
if (!currentPath.includes('login.html') && !currentPath.includes('register.html') && !currentPath.includes('index.html') && currentPath.endsWith('.html')) {
    const loggedInUser = sessionStorage.getItem('user_name');
    if (!loggedInUser) {
...
async function fetchData(endpoint) {
    try {
        ...
        alert('Network Error: Could not connect to the Django backend. Is the server running?');
        return [];
    }
}
